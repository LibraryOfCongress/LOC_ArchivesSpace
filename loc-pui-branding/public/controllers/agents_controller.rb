class AgentsController < ApplicationController
  ...
  def show
    ...
    begin
      ...
      # Ensure the 'Search within results' form is always visible on the agent page
      @search ||= {}
      @search[:dates_within] = false
      @search[:text_within] = true
      
      # If a filter was used and returned no associated records, show a message
      filter_term = Array(params.fetch(:filter_q, [])).reject(&:blank?).first
      if filter_term && @results.blank?
        flash.now[:error] = I18n.t('search_results.no_results')
      end
  ...
  # Fetches associated records for an agent, applying a keyword filter if provided
  def fetch_agent_results(title, uri, params)
    qry = "published_agent_uris:\"#{uri}\" AND types:pui"
    @base_search = "#{uri}?"
    
    search_opts = DEFAULT_AG_SEARCH_OPTS.dup

    filter_term = Array(params.fetch(:filter_q, [])).reject(&:blank?).first
    if filter_term
      # Add the filter term to the options hash, targeting the 'keyword' field
      search_opts['fq'] ||= []
      search_opts['fq'] << "keyword:(#{filter_term})"
    end

    # Call the main helper, which correctly combines the base query with filter
    set_up_search(DEFAULT_AG_TYPES, DEFAULT_AG_FACET_TYPES, search_opts, params, qry)
    ...
    if @results['total_hits'] > 0
      ...
      # If no results, create empty objects to prevent view errors
      @results = []
      @filters = {}
      @facets = []
    end
