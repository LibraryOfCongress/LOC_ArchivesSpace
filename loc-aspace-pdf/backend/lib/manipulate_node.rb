...
# copied from public/app/controllers/concerns/manipulate_node
module ManipulateNode
  ...
  def process_mixed_content_title(text)
    return '' if !text
    return LocMixedContentParser::parse(text, '/')
  end

  def process_mixed_content(in_txt, opts = {})
    # TODO: consider replacing all this with a common mixed content parser
    # return LocMixedContentParser::parse(in_txt, '/')
    return if !in_txt
