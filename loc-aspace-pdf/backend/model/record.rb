  ...
  class Record
        ...
        notes.each do |k, v|
          if k == 'content' || k == 'items'
            ASUtils.wrap(v).each do |s|
              if s.is_a? String
                s.gsub!(/<ref .*?target="(.+?)".*?>(.+?)<\/ref>/m, "<a href='#\\1'>\\2</a>")
              else
    ...
    def parse_notes
      ...
      if json.has_key?('lang_materials')
        ...
        lang_notes = process_json_notes(lang_material_notes.flatten, nil)
    ...
    def parse_dates
      return [] unless (json.has_key?('dates') || json.has_key?('dates_of_existence'))
    ...
    def parse_lang_materials
      return unless json.has_key?('lang_materials')
    ...
    def parse_sub_container_display_string(sub_container, inst, opts = {})
      ...
      instance_type = if inst['instance_type']
                        I18n.t("enumerations.instance_instance_type.#{inst.fetch('instance_type')}", :default => inst.fetch('instance_type'))
                      else
                        nil
                      end
